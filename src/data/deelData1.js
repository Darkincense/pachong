...
function parseHerbString(inputString) {
  ...
  const regex1 = /([\u4e00-\u9fa5]+[\s（]*[\u4e00-\u9fa5]*[,，。.]*[\u4e00-\u9fa5]*[\s）]*)[\s(（]*([一|二|三|四|五|六|七|八|九|十|\d.|适|\s*]+[～[\d]+]*)\s*([克|g|两|半两|两半|钱|半钱|分|斤|量|片|枚]*)?[)）]*/g;
...
const result9 = parseHerbString(input9);
const result10 = parseHerbString(input10);
const result11 = parseHerbString(input11);

...
(async () => {
  ...
  // let writerStream = fs.createWriteStream('dazhongyi.txt');
  // writerStream.write(JSON.stringify(allData), 'UTF8');
  // writerStream.end();

  // 插入数据
  const insertData = async (addSqlParams, index) => {
    const addSql = 'INSERT INTO zhongyao(index_id,name,href,children) VALUES(?,?,?,?)';
  ...
  for (let index = 0; index < allData.length; index++) {
    try {
      ...
      const curData = await itemUl.evaluate(async (e) => {
        ...
        for (let idx0 = 0; idx0 < songList.length; idx0++) {
          ...
          for (let idx1 = 0; idx1 < itemSon.children.length; idx1++) {
            ...
            // console.log('childItem1111', childItem.children)
            ...
            const textHtml = childItem.children.length > 1 && childItem.children[1] && childItem.children[1].innerHTML ? childItem.children[1].innerHTML : '';
            ...
            if (includes(childItem.className, 'usage') || includes(childItem.className, 'fufa')) {
              // 用法用量
              tempObj['usage'] = textHtml;
            }
            if (includes(childItem.className, 'remark')) {
              // 备注
              tempObj['remark'] = textHtml;
            }
            ...
            if (includes(childItem.className, 'provenance')) {
              // 出处
              tempObj['provenance'] = textHtml;
            }
            if (includes(childItem.className, 'habitat')) {
              // 生境分布
              tempObj['habitat'] = textHtml;
            }
            ...
            if (includes(childItem.className, 'functional_indications') || includes(childItem.className, 'zhuzhi')) {
              // 功能主治
              tempObj['functional_indications'] = textHtml;
            }
            
            if (includes(childItem.className, 'care')) {
              // 注意
              tempObj['care'] = textHtml;
            }
            if (includes(childItem.className, 'pharmacological_action')) {
              // 药理作用
              // tempObj['pharmacological_action'] = textHtml;
            }
      ...
      try {
        await insertData([index + 1, allData[index]['name'], allData[index]['href'], allData[index]['children']]);
      } catch(err) {}
